...
class _LinkChildScreenState extends State<LinkChildScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _findByNameEmail() async {
    setState(() => _loading = true);
    final child = await _repo.findChildByNameAndEmail(_nameCtrl.text.trim(), _emailCtrl.text.trim());
    setState(() {
      _foundChild = child;
      _loading = false;
    });
  ...
  Future<void> _sendLinkRequest(String linkMethod) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Link request sent')));
      ...
      body: Column(
        children: [
          TabBar(
            ...
            tabs: const [Tab(text: 'Code'), Tab(text: 'Name & Email'), Tab(text: 'QR')],
          ),
          Expanded(
            child: TabBarView(
              controller: _tabController,
              children: [
                ...
                Padding(
                  ...
                  child: Column(children: [
                    TextField(controller: _codeCtrl, textCapitalization: TextCapitalization.characters, maxLength: 6, decoration: const InputDecoration(labelText: '6-character code')),
                    const SizedBox(height: 12),
                    ElevatedButton(onPressed: _loading ? null : _verifyCode, child: _loading ? const CircularProgressIndicator() : const Text('Verify Code')),
                    const SizedBox(height: 12),
                    if (_foundChild != null) _childPreview(_foundChild!),
                    if (_foundChild != null) ElevatedButton(onPressed: () => _sendLinkRequest('code'), child: const Text('Confirm Link')),
                  ]),
                ),
                ...
                Padding(
                  padding: const EdgeInsets.all(16.0),
                  child: Column(children: [
                    TextField(controller: _nameCtrl, decoration: const InputDecoration(labelText: 'Child Full Name')),
                    const SizedBox(height: 8),
                    TextField(controller: _emailCtrl, decoration: const InputDecoration(labelText: 'Primary Parent Email')),
                    const SizedBox(height: 12),
                    ElevatedButton(onPressed: _loading ? null : _findByNameEmail, child: _loading ? const CircularProgressIndicator() : const Text('Find Child')),
                    const SizedBox(height: 12),
                    if (_foundChild != null) _childPreview(_foundChild!),
                    if (_foundChild != null) ElevatedButton(onPressed: () => _sendLinkRequest('email'), child: const Text('Confirm Link')),
                  ]),
                ),
