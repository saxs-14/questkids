...
class _ChildAnalyticsScreenState extends State<ChildAnalyticsScreen> {
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      ...
      pdf.addPage(pw.Page(
        ...
        build: (ctx) => pw.Column(
          ...
          children: [
            ...
            pw.Text(
                'Grade: ${widget.child.grade} | ${DateTime.now().toString().split('.').first}',
              ...
              pw.Expanded(
                ...
                pw.Text('Avg Score',
                    style: const pw.TextStyle(
                        fontSize: 10, color: PdfColors.grey)),
                pw.Text(
                    '${(_analytics['avgScore'] ?? 0.0).toStringAsFixed(1)}%',
                    style: pw.TextStyle(
                        fontSize: 18, fontWeight: pw.FontWeight.bold)),
              ])),
              pw.Expanded(
      ...
      final file = File(
          '${dir.path}/questkids_${widget.child.name.replaceAll(' ', '_')}.pdf');
  ...
  Widget build(BuildContext context) {
    ...
    final breakdown =
        Map<String, dynamic>.from(_analytics['subjectBreakdown'] ?? {});
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            _StatChip(
                label: '${(_analytics['avgScore'] ?? 0.0).toStringAsFixed(1)}%',
                sub: 'Avg Score'),
            const SizedBox(width: 8),
