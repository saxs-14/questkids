...
class SubjectBarChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (breakdown.isEmpty) return Center(child: Text('No data yet', style: AppTextStyles.bodySmall));
    final entries = breakdown.entries.toList();
    final groups = entries.asMap().entries.map((e) {
      final subj = e.value.key;
      final list = (e.value.value as List).map((v) => (v as num).toDouble()).toList();
      final avg = list.isEmpty ? 0.0 : list.reduce((a, b) => a + b) / list.length;
      return BarChartGroupData(x: e.key, barRods: [
        BarChartRodData(toY: avg, color: _color(subj), width: 18, borderRadius: BorderRadius.circular(6)),
      ]);
    }).toList();
