  ...
  Widget build(BuildContext context) {
    ...
    return ResponsiveScaffold(
      ...
      destinations: const [
        ...
        ResponsiveDestination(
            icon: Icons.group_outlined, activeIcon: Icons.group, label: 'Class'),
        ResponsiveDestination(
                    ...
                    : () async {
                        ...
                        try {
                          ...
                          batch.update(
                            ...
                            {'linkedTeacherUids': FieldValue.arrayUnion([teacherUid])},
                          );
                          batch.update(
                            _db.collection('users').doc(teacherUid),
                            {'linkedChildrenUids': FieldValue.arrayUnion([learnerUid])},
                          );
                          await batch.commit();
...
class _HomeTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot>(
      stream: FirebaseFirestore.instance.collection('users').doc(teacherUid).snapshots(),
      builder: (context, teacherSnap) {
        final linkedUids = teacherSnap.hasData && teacherSnap.data!.exists
            ? List<String>.from(
                (teacherSnap.data!.data() as Map<String, dynamic>?)?['linkedChildrenUids'] ?? [])
            : <String>[];
            ...
            children: [
              ...
              if (linkedUids.isEmpty)
                const _EmptyCard(
                  ...
                  message: 'No learners linked yet. Activity will appear here once learners join.',
                )
              else
                StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, snap) {
                    ...
                    if (docs.isEmpty) {
                      return const _EmptyCard(icon: Icons.assignment_outlined, message: 'No activity recorded yet.');
                    }
                    return Column(
                      children: docs.map((doc) {
                        ...
                        final childName = d['childName'] as String? ?? 'Learner';
              ...
              const Row(
                children: [
                  _QuickActionBtn(label: 'Grade\nReport', icon: Icons.bar_chart_rounded, color: _kPrimary),
                  SizedBox(width: 8),
                  _QuickActionBtn(label: 'Send\nNotification', icon: Icons.send_outlined, color: _kMath),
                  SizedBox(width: 8),
                  _QuickActionBtn(label: 'View\nLeaderboard', icon: Icons.emoji_events_outlined, color: AppColors.gold),
                ],
              ),
              const SizedBox(height: 20),
            ],
...
class _StatsGrid extends StatelessWidget {
  ...
  Future<Map<String, dynamic>> _fetchStats(List<String> uids) async {
    ...
    final todayTs = Timestamp.fromDate(DateTime(today.year, today.month, today.day));
    ...
    return FutureBuilder<Map<String, dynamic>>(
      ...
      builder: (context, snap) {
        ...
        return GridView.count(
          ...
          children: [
            _StatTile(label: 'Total Learners', value: '${linkedUids.length}',
                icon: Icons.people_alt_rounded, color: _kPrimary),
            _StatTile(label: 'Active Today', value: loaded ? '$activeToday' : '…',
                icon: Icons.online_prediction_rounded, color: _kScience),
            _StatTile(label: 'Avg Score %', value: loaded ? '${avgScore.toStringAsFixed(0)}%' : '…',
                icon: Icons.insights_rounded, color: _kMath),
            _StatTile(label: 'Pending Reviews', value: loaded ? '$pending' : '…',
...
class _ClassTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot>(
      stream: FirebaseFirestore.instance.collection('users').doc(teacherUid).snapshots(),
      builder: (context, snap) {
        if (snap.hasError) return Center(child: _ErrorCard(message: snap.error.toString()));
        if (!snap.hasData) return const Center(child: CircularProgressIndicator());
              ...
              child: Column(
                ...
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.group_add_outlined, size: 64, color: _kPrimary),
                  ),
                  const SizedBox(height: 20),
        ...
        return FutureBuilder<List<UserModel>>(
          ...
          builder: (context, learnerSnap) {
            if (learnerSnap.hasError) {
              return Center(child: _ErrorCard(message: learnerSnap.error.toString()));
            }
            if (!learnerSnap.hasData) return const Center(child: CircularProgressIndicator());
...
class _LearnerCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final initials = learner.name.isNotEmpty
        ? learner.name.split(' ').map((w) => w.isNotEmpty ? w[0] : '').take(2).join()
        : '?';
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 6),
                      Text('Last active ${_lastActiveFmt()}', style: AppTextStyles.bodySmall),
                    ],
                  ),
                  const SizedBox(height: 8),
                  Stack(
                    children: [
                      ...
                      FractionallySizedBox(
                        ...
                        child: Container(
                          height: 6,
                          decoration: BoxDecoration(
                            gradient: const LinearGradient(colors: [_kPrimary, _kScience]),
                            borderRadius: BorderRadius.circular(4),
                          ),
                        ),
                      ),
                  ...
                  const SizedBox(height: 4),
                  Text('${learner.totalPoints} / 1000 XP', style: AppTextStyles.bodySmall),
                ],
              ),
            ),
      ...
      builder: (_, scrollCtrl) => ListView(
        ...
        children: [
            ...
            children: [
              CircleAvatar(
                ...
                child: learner.avatarUrl == null
                    ? Text(initials.toUpperCase(),
                        style: const TextStyle(
                            color: _kPrimary, fontWeight: FontWeight.w800, fontSize: 20))
                    : null,
              ),
          ...
          Row(
            children: [
              Expanded(
                child: _MetricTile(label: 'Total XP', value: '${learner.totalPoints}',
                    icon: Icons.bolt, color: _kPrimary),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: _MetricTile(label: 'Streak', value: '${learner.streakDays} days',
                    icon: Icons.local_fire_department_rounded, color: _kMath),
              ),
            ],
          ),
          const SizedBox(height: 24),
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<QuerySnapshot>(
      ...
      builder: (context, snap) {
        ...
        final defaults = <String>['Mathematics', 'Science', 'English', 'Social Sciences'];
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Text(
                        scores.isEmpty ? 'No data' : '${avg.toStringAsFixed(0)}%',
                        style: AppTextStyles.bodySmall.copyWith(
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<QuerySnapshot>(
      ...
      builder: (context, snap) {
        ...
        final docs = snap.data!.docs;
        if (docs.isEmpty) {
          return const _EmptyCard(icon: Icons.sports_esports_outlined, message: 'No games played yet.');
        }
        return Column(
          children: docs.map((doc) {
            final d = doc.data() as Map<String, dynamic>;
                    ...
                    children: [
                      Text('$score%',
                          style: AppTextStyles.bodyMedium.copyWith(
                              color: color, fontWeight: FontWeight.w700)),
                      Text(_timeAgo(d['playedAt']), style: AppTextStyles.bodySmall),
                    ],
                  ),
                ],
              ),
              ...
              child: Column(
                ...
                children: [
                  Container(
                    padding: const EdgeInsets.all(28),
                    decoration: BoxDecoration(
                        color: _kPrimary.withValues(alpha: 0.08), shape: BoxShape.circle),
                    child: const Icon(Icons.assignment_add, size: 64, color: _kPrimary),
                  ),
                  const SizedBox(height: 20),
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Text(_fmtDate(data['createdAt']), style: AppTextStyles.bodySmall),
                  ],
                ),
                const SizedBox(height: 12),
                Row(
...
class _CreateActivitySheetState extends State<_CreateActivitySheet> {
  ...
  static const _subjects = [
    'Mathematics', 'Science', 'English', 'Social Sciences', 'Technology', 'Arts & Culture',
  ];
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Activity created!'), backgroundColor: AppColors.success),
        );
      }
    } catch (e) {
      if (mounted) {
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              decoration: InputDecoration(
                ...
                prefixIcon: Padding(
                  ...
                  child: Container(
                    width: 12, height: 12,
                    decoration: BoxDecoration(color: _subjectColor, shape: BoxShape.circle),
                  ),
                ),
              ),
            ...
            DropdownButtonFormField<String>(
              ...
              onChanged: (v) => setState(() => _selectedGrade = v ?? _selectedGrade),
            ),
            const SizedBox(height: 14),
        ...
        children: [
          ...
          _ProfileInfoCard(
            children: [
              ...
              _InfoRow(icon: Icons.school_outlined, label: 'Grade / Class',
                  value: currentUser?.grade ?? 'Not set'),
              const Divider(height: 1),
              const _InfoRow(icon: Icons.person_outline, label: 'Role', value: 'Teacher'),
              const Divider(height: 1),
              _InfoRow(icon: Icons.language_outlined, label: 'Language',
            ...
            _ProfileInfoCard(
              children: [
                _InfoRow(
                  ...
                  valueStyle: AppTextStyles.h4.copyWith(letterSpacing: 4, color: _kPrimary),
                ),
              ],
            ),
            const SizedBox(height: 20),
...
class _ActivityCard extends StatelessWidget {
  ...
  const _ActivityCard({
    required this.learnerName, required this.subject,
    required this.score, required this.timeAgo, required this.subjectColor,
  });
        ...
        children: [
          CircleAvatar(
            ...
            child: Text(
              ...
              style: TextStyle(color: subjectColor, fontWeight: FontWeight.w700),
            ),
          ),
          const SizedBox(width: 12),
          Expanded(
