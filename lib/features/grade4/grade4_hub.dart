...
class _Grade4HubState extends State<Grade4Hub> {
  ...
  Future<void> _load() async {
    ...
    final daily = await _repo.getOrCreateDailyMissions(
        uid, widget.user?.grade ?? 'Grade 4');
        ...
        Container(
          ...
          child: Row(children: [
            CircleAvatar(
                ...
                child: Text(widget.user?.avatarEmoji ?? '🙂',
            ...
            Expanded(
                child: Column(
                    ...
                    children: [
                  Text(widget.user?.name ?? 'Learner',
                      style: AppTextStyles.h2.copyWith(color: Colors.white)),
                  const SizedBox(height: 6),
                  Text('Grade 4 • Level $level $rank',
                        ...
                        ...(((_daily?['missions'] ?? []) as List)
                            .map((m) => _MissionRow(
                                ...
                                onTap: () async {
                                  ...
                                  try {
                                    await _repo.updateMissionProgress(
                                        uid, m['id'], 1);
                                    final refreshed =
                                    ...
                                    messenger.showSnackBar(SnackBar(
                                        content: Text(
                                            'Error updating mission: $e')));
                                  }
                                }))),
                        const SizedBox(height: 8),
                        Text('+ rewards shown on completion',
                            style: AppTextStyles.bodySmall
                  ...
                  onTap: () {
                    if (unlocked) {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (_) => TugOfWarScreen(
                                worldId: worldId, user: widget.user)),
                      );
                    } else {
                      ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('World locked')));
                    }
                  },
                  child: Container(
                    width: 200,
        ...
        Card(
            child: Column(
                children: _leaderboard
                    .map((e) => ListTile(
                        leading: Text(e['avatarEmoji'] ?? '🙂'),
                        title: Text(e['name'] ?? ''),
                        trailing:
                            Text('Lv ${e['level'] ?? 1} • ${e['xp'] ?? 0} XP')))
                    .toList())),
      ]),
    );
  }
